...
class TraderCurrency(BackTestTrader):
    ...
    def open_buy(self, code, opt: Operation, amount: float = None):
        try:
            ...
            if res is False:
                utils.send_telegram_message("currency", "数字货币交易提醒", f"{code} open buy 下单失败")
                return False
            msg = f"开多仓 {code} 价格 {res['price']} 数量 {open_usdt} 原因 {opt.msg}"
            utils.send_telegram_message("currency", "数字货币交易提醒", msg)
            ...
            utils.send_telegram_message("currency", "数字货币交易提醒", f"{code} open buy 异常: {str(e)}")
            return False
    ...
    def open_sell(self, code, opt: Operation, amount: float = None):
        try:
            ...
            if res is False:
                utils.send_telegram_message("currency", "数字货币交易提醒", f"{code} open sell 下单失败")
                return False
            ...
            utils.send_telegram_message("currency", "数字货币交易提醒", f"{code} open sell 异常: {str(e)}")
            return False
    ...
    def close_buy(self, code, pos: POSITION, opt: Operation):
        try:
            ...
            if res is False:
                utils.send_telegram_message("currency", "数字货币交易提醒", f"{code} 下单失败")
                return False
            ...
            utils.send_telegram_message("currency", "数字货币交易提醒", f"{code} close buy 异常: {str(e)}")
            return False
    ...
    def close_sell(self, code, pos: POSITION, opt: Operation):
        try:
            ...
            utils.send_telegram_message("currency", "数字货币交易提醒", f"{code} close sell 异常: {str(e)}")
            return False
