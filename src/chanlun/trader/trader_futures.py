...
class TraderFutures(BackTestTrader):
    ...
    def open_buy(self, code, opt: Operation, amount: float = None):
        try:
            ...
            if res is False or res["price"] is None:
                utils.send_telegram_message("futures", "期货交易提醒", f"{code} open long 下单失败")
                return False

            stock_info = self.ex.stock_info(code)

            msg = f"开多仓 {code} 价格 {res['price']} 数量 {self.unit_volume} 原因 {opt.msg}"
            utils.send_telegram_message("futures", "期货交易提醒", msg)
            ...
            utils.send_telegram_message("futures", "期货交易提醒", f"{code} open long 异常: {str(e)}")
            return False
    ...
    def open_sell(self, code, opt: Operation, amount: float = None):
        try:
            ...
            if res is False or res["price"] is None:
                utils.send_telegram_message("futures", "期货交易提醒", f"{code} open short 下单失败")
                return False
            ...
            utils.send_telegram_message("futures", "期货交易提醒", f"{code} open short 异常: {str(e)}")
            return False
    ...
    def close_buy(self, code, pos: POSITION, opt: Operation):
        try:
            ...
            if res is False or res["price"] is None:
                utils.send_telegram_message("futures", "期货交易提醒", f"{code} 下单失败")
                return False
            ...
            utils.send_telegram_message("futures", "期货交易提醒", f"{code} close buy 异常: {str(e)}")
            return False
    ...
    def close_sell(self, code, pos: POSITION, opt: Operation):
        try:
            ...
            utils.send_telegram_message("futures", "期货交易提醒", f"{code} close sell 异常: {str(e)}")
            return False
